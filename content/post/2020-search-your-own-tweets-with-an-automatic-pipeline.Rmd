---
title: Search your own tweets with an automatic pipeline
author: Thomas
date: '2020-07-08'
slug: 2020-search-your-own-tweets-with-an-automatic-pipeline
categories: []
tags: []
...
But the difficulties come after, when I when to search back into the tweets I liked. I have to scroll a lot in my profile page and click to see more tweets. That's harassing. So I design a little tool that could help my search inside my own tweets : [Morphea](https://morphea.netlify.app/).
...
The big part in this pipeline is to manage all of the keys and tokens. That was the most difficult part because you have to deal with many API. For that, I used the secrets in Github Settings. 
...
If needed, a more details [how-to](https://www.hvitfeldt.me/blog/bookdown-netlify-github-actions/) has been written by Emil Hvitfeldt.

## The final result

### A big table

All of this results in a [big table](https://morphea.netlify.app/) with approximately 1.000 tweets inside (100 per page), generated through `rmarkdown` for the document and `reactable` for the table itself. I recommend this awesome package for creating interactive tables, have a look at [it](https://glin.github.io/reactable/index.html).
...
+ link to main resources : you have a link to the tweet on the left (the anchor) and links to the main media (including media in quotes) and main urls in each tweet.

Note that you can also use [Twitter Embedding](https://developer.twitter.com/en/docs/twitter-for-websites/embedded-tweets/overview) to print nice formatted tweets. But it doesn't work well [here](https://github.com/glin/reactable/issues/55) and reduce the search possibilities.
